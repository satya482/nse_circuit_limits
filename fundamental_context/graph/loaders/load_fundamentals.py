...
def _quality_score(
    ...
    if roce is not None and roce > 30:
        pts += 2
    if ob_ratio is not None and ob_ratio > 2:
        pts += 3
...
def _conviction_business_quality(
    roe: float | None,
    roce: float | None,
    ob_ratio: float | None,
    pledge: float | None,
...
def parse_csv(path: Path) -> list[dict]:
    ...
    with open(path, encoding="utf-8-sig") as fh:
        ...
        for raw in reader:
            ...
            pledge = _safe_float(
                raw.get("Promoter holding pledge percentage % Qtr", "")
            )
            ...
            conv_init = _conviction_business_quality(
                roe, roce, ob_ratio, pledge, promoter, debt_eq
            )
