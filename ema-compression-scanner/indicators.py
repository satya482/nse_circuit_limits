...
def _atr(
    high: pd.Series, low: pd.Series, close: pd.Series, period: int, wilder: bool = False
) -> pd.Series:
    tr = pd.concat(
        [
            high - low,
...
def bollinger_keltner(
    ...
    bb_roll = df["bb_width"].rolling(
        lookback, min_periods=max(lookback // 2, bb_period)
    )
    bb_min = bb_roll.min()
    bb_max = bb_roll.max()
...
def squeeze_stats(
    df: pd.DataFrame, squeeze_min_bars: int, bb_width_pct_max: float
) -> tuple[bool, int]:
    """
