...
def build_markdown(
    ...
    for i, c in enumerate(candidates, 1):
        ...
        df = c["df"]
        prev_close = (
            float(df.iloc[-2]["close"]) if len(df) >= 2 else float(last["close"])
        )
        day_chg_pct = (
...
def run():
    ...
    print(
        f"[{datetime.now().strftime('%H:%M:%S')}] EMA Compression + BB Squeeze Scanner starting..."
    )
    ...
    for c, total_score in zip(raw_candidates, scores):
        ...
        vol_ctxt = (
            round(max(0.0, 1.0 - recent_vol / long_vol) * 100, 1)
            if long_vol > 0
            else 50.0
        )
        ...
        components = {
            "ema_tightness": round(
                100
                / max(spread_atr, 0.01)
                / (100 / max(g["ema_spread_atr_ratio"], 0.01))
                * 100,
                1,
            ),
            ...
            "rs_strength": round(
                (rw["gap"] * c["rs_gap"] + rw["slope"] * c["rs_slope"]) * 100, 2
            ),
        }
