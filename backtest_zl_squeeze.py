...
def run(symbol: str, lookback: int = 600) -> None:
    ...
    if has_rs:
        _print_trades(
            symbol, trades_no_rs, label="Without RS filter (ZL + Squeeze only)"
        )
...
def _print_episodes(symbol: str, ep_rows: list, has_rs: bool) -> None:
    ...
    print(
        "  Sqz = squeeze bars   SqzRS = squeeze AND RS passing   * = entry signal taken"
    )
    print(f"{'='*w}")
    print(
        f"  {'Start':12} {'End':12} {'Bars':>5}  {'Sqz':>4} {'SqzRS':>6}  Squeeze dates (while ZL rising)"
    )
...
def _trade_summary(trades: list) -> None:
    ...
    if wins:
        print(
            f"  Avg win:     +{sum(t['pnl_pct'] for t in wins)/len(wins):.1f}%"
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        description="ZL + Squeeze + RS Backtest — any NSE symbol"
    )
    ap.add_argument("symbol", help="NSE symbol, e.g. KRN")
    ap.add_argument(
        "--lookback", type=int, default=600, help="bars of history (default 600)"
    )
    args = ap.parse_args()
