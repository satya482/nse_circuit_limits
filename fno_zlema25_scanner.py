...
def _bb_kc_squeeze(df: pd.DataFrame) -> bool:
    ...
    return bool(
        (bb_basis + 2.0*bb_std).iloc[-1] < (kc_basis + 1.5*kc_atr).iloc[-1] and
        (bb_basis - 2.0*bb_std).iloc[-1] > (kc_basis - 1.5*kc_atr).iloc[-1]
    )


def _last_turn(zl25: pd.Series, closes: pd.Series, direction: str) -> tuple[int, float] | None:
    """
    ...
    for i in range(n - 1, limit - 1, -1):
        if i < 2:
            break
        if direction == "up":
            turned = zl25.iloc[i] > zl25.iloc[i-1] and zl25.iloc[i-1] <= zl25.iloc[i-2]
        else:
            turned = zl25.iloc[i] < zl25.iloc[i-1] and zl25.iloc[i-1] >= zl25.iloc[i-2]
        if turned:
            bars_ago = (n - 1) - i + 1
...
def _analyse(symbol: str) -> dict | None:
    try:
        ...
        return {
            ...
            "day_chg":   round((float(c.iloc[-1]) - float(c.iloc[-2])) / float(c.iloc[-2]) * 100, 2),
            "zl_rising": bool(zl25.iloc[-1] > zl25.iloc[-2]),
            "zl_dist":   round((float(c.iloc[-1]) - float(zl25.iloc[-1])) / float(zl25.iloc[-1]) * 100, 2),
...
def _print_summary(up_starts: list[dict], dn_starts: list[dict]) -> None:
    ...
    def _fmt(f: dict, key: str) -> str:
        ...
        return (f"  {f['symbol']:<14} {f['close']:>9.2f}  "
