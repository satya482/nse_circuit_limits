...
function buildAnnotations(type, n) {{
  ...
  if (type === 'sma200') {{
    ...
    if (_showCross) {{
      const s10 = sl(DATA.sma10, n);
      const s20 = sl(DATA.sma20, n);
      for (let i = 1; i < s10.length; i++) {{
        if (s10[i] == null || s20[i] == null || s10[i-1] == null || s20[i-1] == null) continue;
        if (s10[i] > s20[i] && s10[i-1] <= s20[i-1]) {{
          annot['xo'+i] = {{ type: 'point', xValue: dates[i], yValue: (s10[i]+s20[i])/2,
            pointStyle: 'circle', radius: 4, backgroundColor: '#1fd980', borderColor: '#1fd980', borderWidth: 1.5 }};
        }} else if (s10[i] < s20[i] && s10[i-1] >= s20[i-1]) {{
          annot['xu'+i] = {{ type: 'point', xValue: dates[i], yValue: (s10[i]+s20[i])/2,
            pointStyle: 'circle', radius: 4, backgroundColor: '#ff5577', borderColor: '#ff5577', borderWidth: 1.5 }};
        }}
      }}
    }}
    return annot;
...
function toggleCross() {{
  ...
  document.getElementById('btn-cross').classList.toggle('on', _showCross);
  chartSma200.options.plugins.annotation.annotations = buildAnnotations('sma200', _range);
  chartSma200.update('none');
}}
