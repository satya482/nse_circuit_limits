...
# run_all_scanners.ps1 to run EMA55_Cross AFTER all four of these so today's
# files already exist.
...
RS_WEEKLY_EMA9_MD = os.path.join(REPO_DIR, "rs_weekly_scans", "rs_weekly_ema9_scans.md")
...
def _table_rows(findings: list[dict], circuit: dict[str, tuple], confluence_syms: set[str]) -> list[str]:
    ...
    for f in findings:
        ...
        marker = "🟢 " if sym in confluence_syms else ""
...
def build_markdown(findings: list[dict], circuit: dict[str, tuple]) -> str:
    ...
    weekly_rs_syms, weekly_rs_note = _load_union_source(
        RS_WEEKLY_EMA9_MD, {"INDICES", "COMMODITIES"}, "Weekly RS EMA9"
    )
    ...
    if weekly_rs_syms is not None:
        source_sets["Weekly RS EMA9"] = weekly_rs_syms
    union_notes = [
    ...
    if rows:
        ...
        lines += hdr + _table_rows(rows, circuit, confluence_syms)
